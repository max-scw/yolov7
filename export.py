...
if __name__ == '__main__':
    ...
    #     filename = Path(opt.weights).with_suffix(".torchscript.pt")  # filename
    #     ts = torch.jit.trace(model, img, strict=False)
    #     ts.save(filename)
    #     print(f"TorchScript export success; saved as {filename.as_posix()}")
    ...
    #
    #     filename = Path(opt.weights).with_suffix(".mlmodel")  # filename
    #     ct_model.save(filename)
    ...
    # try:
    #     print("\nStarting TorchScript-Lite export with torch {torch.__version__} ...")
    #     filename = Path(opt.weights).with_suffix(".torchscript.ptl")  # filename
    #     tsl = torch.jit.trace(model, img, strict=False)
    #     tsl = optimize_for_mobile(tsl)
    #     tsl._save_for_lite_interpreter(filename)
    #     print(f"TorchScript-Lite export success; saved as {filename}")
    # except Exception as e:
    #     print(f"TorchScript-Lite export failure: {e}")
    ...
    try:
        ...
        # output (? x 7) [batch_index, x1, y1, x2?, y2?, class_id, conf]
        ...
        if opt.end2end and opt.max_wh is None:
            for i in onnx_model.graph.output:
                for j in i.type.tensor_type.shape.dim:
                    j.dim_param = str(shapes.pop(0)) # FIXME: if --grid is not set, this can be reached without "shapes" being defined
